...
COURSES = ("大学英语", "大学生心理健康教育", "线性代数", "数据结构", "军事理论", "中国近现代史纲要")
REQUIRED_ROOT_FILES = (
...
MENTAL_HEALTH_FILES = ("README.md", "文件说明.md")
MENTAL_HEALTH_DIRS = ("考试",)
...
def _check_linear_algebra(root: Path, result: AuditResult) -> None:
    ...
    for value in LINEAR_ALGEBRA_FILES:
        ...
        if not path.is_file():
            result.add("required-course-file", path.relative_to(root), "required linear-algebra file is missing")
    for value in LINEAR_ALGEBRA_DIRS:
        path = course_root / value
        if not path.is_dir():
    ...
    for target in ("文件说明.md", "考试/", "练习与题库/", "知识点/"):
...
def _check_mental_health(root: Path, result: AuditResult) -> None:
    course_root = root / "大学生心理健康教育"
    for value in MENTAL_HEALTH_FILES:
        path = course_root / value
        if not path.is_file():
            result.add("required-course-file", path.relative_to(root), "required mental-health file is missing")
    for value in MENTAL_HEALTH_DIRS:
        path = course_root / value
        if not path.is_dir():
            result.add("required-course-directory", path.relative_to(root), "required resource directory is missing")
...
def _check_required_content(root: Path, result: AuditResult) -> None:
    ...
    _check_mental_health(root, result)
